# https://youtu.be/bARCE45FM4s
# he shows a picture during less than a second around 6:03
...
def get_crypto_data(pair, since):
    ret = k.query_pubilc('OHLC', data = { 'pair': pair, 'since': since })
...
def check_data(name, crypto_data, should_buy):
    # TODO: dont print repeat-print if list too short
    high = 0
    close = 0
    low = 0
    ...
    # OBS !!! fix this
    # add every moving average into the same array
    mva[name]['high'].appned(high / 100)
    mva[name]['low'].appned(low / 100)
    mva[name]['close'].appned(close / 100)
    #print(name)
...
def try_buy(data, name, crypto_data):
    # analyse the data to see if it is good opportunity to buy
...
def try_sell(data, name, crypto_data):
    # analyse the data to see if it is good opportunity to sell
